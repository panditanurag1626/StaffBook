...
// Predefined button style configurations matching ConnectButton exactly
export const APPLY_BUTTON_STYLES = {
  default: {
    className: "h-12 sm:h-10 w-36 sm:w-32 px-6 text-sm",
    iconSize: 16,
  },
  compact: {
    // Used in job cards - matches ConnectButton compact size exactly
...
const ApplyButton: React.FC<ApplyButtonProps> = ({ 
  ...
  const sizeConfig = APPLY_BUTTON_STYLES[size];
  ...
  return (
    <button 
      ...
      {...props}
    >
      {icon || <FiSend size={sizeConfig.iconSize} />}
      <span>{label}</span>
    </button>
  );
};
